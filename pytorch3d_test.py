...
if __name__ == '__main__':
    ...
    for i in range(0, epochs):
        ...
        est = BF_EST.Beamform(RP_EST, BI=range(0, len(batch)), soft=True, z=z).abs().to(dev_1)
        GT = BF_GT.Beamform(RP_GT, BI=range(0, len(batch)), soft=True, z=z).abs().to(dev_1)
        est_norm = est / torch.norm(est, p=1)
        GT_norm = GT / torch.norm(GT, p=1)

        L1 = torch.sum(torch.abs(est_norm - GT_norm))
        ...
        loss = w_L1 * L1 + w_laplacian * loss_laplacian + w_edge * loss_edge + w_normal * loss_normal
        ...
        vis.line(X=torch.ones((1)).cpu() * i, Y=loss.unsqueeze(0).cpu(), win=loss_window, name='L1', update='append')
