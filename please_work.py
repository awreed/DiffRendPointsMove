...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        #src_mesh = getMesh(path=path, device=dev, scale=-0.1)

        #src_mesh = ico_sphere(4, dev)
        #src_mesh = src_mesh.scale_verts(0.3)
        # Define objects in scene
        #ps_GT = torch.tensor([[-.2, -.2, .01]], requires_grad=False).to(dev)
        #ps_EST = torch.tensor([[.2, 0.2, 0]], requires_grad=False).to(dev)
        ...
        #RP.generateImpulse()
        RP.generateTransmitSignal(compute=False)
    ...
    ps_est = ps_EST.clone()
    #deform_verts = torch.full(ps_est.verts_packed().shape, 0.0, device=dev, requires_grad=True)
    ...
    for i in range(1, 100000):
        ...
        #sample_trg = sample_points_from_meshes(trg_mesh, numPoints, True)
        #sample_src = sample_points_from_meshes(new_src_mesh, numPoints, True)

        #ps_GT = sample_trg[0].squeeze().to(RP.dev)
        #ps_GT_normals = sample_trg[1].squeeze().to(RP.dev)

        #print(ps_GT.shape)
        #print(ps_GT_normals.shape)

        #ps_est = sample_src[0].squeeze().to(RP.dev)
        #ps_est_normals = sample_src[1].squeeze().to(RP.dev)
        print(RP.pixPos3D)
        ...
        with torch.no_grad():
            ...
            GT = BF.Beamform(RP, BI=range(0, len(batch)), soft=False, pixels=RP.pixPos3D).abs().to(dev_1)
        ...
        #simulateWaveformsBatched(RP, ps_est, batch, propLoss=propLoss)
            ...
            est = BF.Beamform(RP, BI=range(0, len(batch)), soft=False, pixels=RP.pixPos3D).abs().to(dev_1)
        ...
        loss_chamfer, _ = chamfer_distance(ps_est.unsqueeze(0), ps_GT.unsqueeze(0).to(dev))
